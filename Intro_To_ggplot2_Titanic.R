...
titanic <- read.csv("C:/temp/titanic.csv")
#or  
...
#REAL EXAMPLE
# to know more about the mpg data
?mpg
ggplot(data = mpg) + 
...
# you may have a warning about non-finite values. Non-finite suggests a divison by zero.
ggplot(titanic, aes(x = Age, fill = Survived)) +
...
ggplot(titanic, aes(x = Age, y=(..count..)/sum(..count..),fill = Survived)) +
  geom_histogram(binwidth = 5,color = "white") +
...
ggplot(titanic, aes(x = Age, fill = Survived)) +
  geom_histogram(binwidth = 5,color = "white") +
  labs(x = "Age",y = "Passengers", title = "Passenger Survival Count by age") +
